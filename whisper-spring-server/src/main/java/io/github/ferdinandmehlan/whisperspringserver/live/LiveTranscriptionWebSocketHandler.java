...
/**
 * WebSocket handler for live audio transcription.
 * Manages WebSocket sessions, forwarding incoming binary audio data to
 * {@link LiveTranscriptionSession} instances for processing.
 */
public class LiveTranscriptionWebSocketHandler extends BinaryWebSocketHandler {
    ...
    public void afterConnectionEstablished(WebSocketSession webSocketSession) {
        ...
        LiveTranscriptionSession session =
                new LiveTranscriptionSession(webSocketSession.getId(), model, waveService, options, transcription -> {
                    ...
                    if (webSocketSession.isOpen() && !text.isBlank()) {
                        try {
                            ...
                            log.error(
                                    "Failed to send transcription result for session {}", webSocketSession.getId(), e);
                        }
                    }
    ...
    private void removeSession(String sessionId) {
        LiveTranscriptionSession session = activeSessions.remove(sessionId);
        if (session != null) {
            session.close();
        }
    }
