...
import io.github.ferdinandmehlan.whisperspring._native.callbacks.CallbackRegistry;
...
public class WhisperFullParams {
     ...
     * @param callbackRegistry the callback registry for native callbacks
     * @return the allocated memory segment
     */
    public static MemorySegment allocate(
            Arena arena, WhisperTranscribeConfig config, CallbackRegistry callbackRegistry) {
        ...
        if (callbackRegistry != null) {
            NEW_SEGMENT_CALLBACK.set(
                    segment, 0L, callbackRegistry.registerNewSegmentCallback(config.newSegmentCallback));
            PROGRESS_CALLBACK.set(segment, 0L, callbackRegistry.registerProgressCallback(config.progressCallback));
            ENCODER_BEGIN_CALLBACK.set(
                    segment, 0L, callbackRegistry.registerEncoderBeginCallback(config.encoderBeginCallback));
        }
